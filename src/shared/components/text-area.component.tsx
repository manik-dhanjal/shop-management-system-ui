...
const TextArea = ({ label, value, className, ...otherProps }: TextAreaType) => {
	return (
		<div className={`relative mt-3 ${className}`}>
			<textarea
				className="text-md border dark:bg-gray-800 border-transparent dark:border-gray-700/60 text-gray-700 dark:text-gray-300 bg-white rounded-lg overflow-hidden px-5 pt-6 w-full outline-none focus:border-slate-500 peer  h-[150px] shadow-sm"
				id={otherProps.name}
				value={value}
				{...otherProps}
			/>
